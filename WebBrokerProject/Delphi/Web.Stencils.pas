...
begin
  ...
  begin
    ...
    if LField = nil then
    begin
      LType := FRTTIContext.GetType(LObj.ClassInfo);
...
begin
  try
    ...
    LExpression := TBindings.CreateExpression(Slice(LScopes, i), AExpression);
    try
...
reuse_token:
    ...
    if LParsedTemplate <> '' then
    begin
      Result := ReplaceSymbol(LParsedTemplate, '@RenderBody', Result);
      Result := ReplaceSymbol(Result, '@RenderHeader', LParsedExtraHeader);

      // collect secondary pages content and place it
      LAddedPagesText := '';
      for I := 0 to FAddedPages.Count - 1 do
        LAddedPagesText := LAddedPagesText + FAddedPages.Strings[I];
      Result := ReplaceSymbol(Result, '@RenderPages', LAddedPagesText);
    end;
  finally
    LOutStream.Free;
  end;
...
begin
  ...
  try
    ...
    LPage.Values['request_path'] := LReqPath;                    // <-- ADD THIS LINE
    LPage.Values['request_segment'] := ExtractLastSegment(LReqPath); // <-- AND THIS LINE
    if WebRequest <> nil then
    begin
...
type
  TWebStencilsDataSetScope = class(TInterfacedObject, IScope)
  private type
    TFieldValue = class(TInterfacedObject, IValue)
    private
      FField: TField;
    protected
...
type
  TWebStencilsStringsScope = class(TInterfacedObject, IScope)
  private
    FStrings: TStrings;
    FIndex: Integer;
    FObjectScope: IScope;
...
type
  TWebStencilsDeferredScope = class(TInterfacedObject, IScope)
  private
    FComponent: IWebStencilsComponent;
    FDictItem: TWebStencilsDataVar;
